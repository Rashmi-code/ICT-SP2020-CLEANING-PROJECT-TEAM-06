...
$query_Recordset1 = "SELECT * FROM ims_usrpass where service!='admin' ORDER BY`sno` DESC ";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO ims_usrpass (name, phno, username, password, status, service, remarks) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['name'], "text"),
                       GetSQLValueString($_POST['phno'], "text"),
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO products (name) VALUES (%s)",
                       GetSQLValueString($_POST['name'], "text"));
...
<?php if(@$_GET['s']=="s"){print("<script>alert('Data Submitted Sucessfully!');</script>");echo('<script>(location.href="add_user.php");</script>');} ?>

<table width="100%" border="0" cellspacing="0" cellpadding="0" class="stdtable">
  <form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST">
        
    <tr>
      <td><label for="Size2" class="marginleft10">Name </label></td>
      <td><input name="name" type="text" class="smallinput" id="name" required /></td>
    </tr>
    <tr>
      <td><label for="Size2" class="marginleft10">Ph No.</label></td>
      <td><input name="phno" type="text" class="smallinput" id="phno"  /></td>
    </tr>
    <tr>
      <td><label for="Size2" class="marginleft10">Username </label></td>
      <td><input name="username" type="text" class="smallinput" id="username" required /></td>
    </tr>
    <tr>
      <td width="29%"><label for="name" class="marginleft10">Designation</label></td>
    ...
    <tr>
      <td><label for="Size2" class="marginleft10">Password</label></td>
      <td><input  name="password" type="text" class="smallinput" id="password" required/></td>
    </tr>
    <tr>
  ...
  <tr>
    <td colspan="2"><table width="76%" border="0" cellspacing="0" cellpadding="0">
      <tr class="external-event">
        <th width="19%" height="41" class="head0"><h5>Name (Username)</h5></th>
      
        <th width="34%" class="head0"><h5>Phno</h5></th>
        <th width="34%" class="head0"><h5>Service</h5></th>
        <th width="6%" class="head1"><h5>Edit</h5></th>
        <th width="8%" class="head1"><h5>Delete</h5></th>
        </tr>
     
      <?php do { ?>
        <tr>
          <td><?php echo $row_Recordset1['name']." "."(" .$row_Recordset1['username'].")"; ?></td>
          <td><?php echo $row_Recordset1['phno']; ?></td>
          <td><?php echo $row_Recordset1['service']; ?></td>
          <td><a href="edit_user.php?id=<?php echo $row_Recordset1['sno']; ?>" ><img src="images/edit.png" width="16" height="16" /></a></td>
          <td><a href="del_user.php?id=<?php echo $row_Recordset1['sno']; ?>"  onclick="javascript:return confirm('Are you really want to delete?')"><img src="images/de;.png" width="16" height="16" /></a></td>
        </tr>
