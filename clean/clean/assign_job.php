...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE cleaning SET rooms=%s, bathrooms=%s, Balcony=%s, Kitchen_Cupboards=%s, Fridge=%s, Oven=%s, Pantry=%s, After_Party=%s, Carpet_Steam_Clean=%s, Internal_Windows=%s, dateof=%s, timeof=%s, name=%s, phone=%s, email=%s, address=%s, type=%s, enteredby=%s, assign=%s WHERE id=%s",
...
mysqli_select_db($dbconnection, $database_dbconnection);
$query_Recordset2 = "SELECT username, name FROM ims_usrpass";
...
<form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="stdtable">
     ...
     <?php do { ?>      <option value="<?php echo $row_Recordset2['username']; ?>"> <?php echo $row_Recordset2['username']  ?></option>
      <?php } while ($row_Recordset2 = mysqli_fetch_assoc($Recordset2)); ?> </select></td>
